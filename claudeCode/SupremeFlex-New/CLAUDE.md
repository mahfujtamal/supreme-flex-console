...
11. B2B orders are initiated and delivered by KAM. B2C default delivery agent is DH; channel/sub-channel may override with own delivery (default or per-order).
12. Every order must reference a specific connection via `anchor_id` + `active_service_id` — never attach an order to a customer alone.
13. Hub Managers do not exist in this system. Each delivery entity (DH, Channel, Sub-Channel) has a manager user linked via `manager_admin_id`. Field Agents report directly to their DH. KAMs are independent.
