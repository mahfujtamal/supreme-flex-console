...
class OttOrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = DB::table('ott_order_history')->orderByDesc('created_at');
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'ott_product_id'    => 'required|uuid',
        ]);

        $id = Uuid::make();
        DB::table('ott_order_history')->insert([
            'id'                => $id,
            ...
            'ott_product_id'    => Uuid::toBin($request->ott_product_id),
            'status'            => 'PENDING',
            'notes'             => $request->notes,
            'created_at'        => now(),
            'updated_at'        => now(),
        ...
        return response()->json(BaseApiController::castRecord(DB::table('ott_order_history')->where('id', $id)->first()), 201);
    }

    public function show(string $id)
    {
        $record = DB::table('ott_order_history')->where('id', Uuid::toBin($id))->first();
    ...
    {
        $binId = Uuid::toBin($id);
        $record = DB::table('ott_order_history')->where('id', $binId)->first();
        if (!$record) return response()->json(['message' => 'Not found'], 404);

        $request->validate(['status' => 'required|in:PENDING,ACTIVE,CANCELLED']);

        DB::table('ott_order_history')->where('id', $binId)->update([
        ...
        return response()->json(BaseApiController::castRecord(DB::table('ott_order_history')->where('id', $binId)->first()));
    }
}
