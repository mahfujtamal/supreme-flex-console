...
export default function NetworkZonesTab() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (editId) {
        const { error } = await supabase.from("network_zones").update({ network_zone_name: name }).eq("network_zone_id", editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("network_zones").insert({ network_zone_name: name });
        if (error) throw error;
      }
    },
    onSuccess: () => {
  ...
  const openEdit = (item: any) => { setEditId(item.network_zone_id); setName(item.network_zone_name); setOpen(true); };
    ...
    <div className="space-y-4">
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">No zones found.</TableCell></TableRow>
            ) : data.items.map((z) => (
              <TableRow key={z.network_zone_id}>
                ...
                <TableCell>
                  <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEdit(z)}><Pencil className="h-3.5 w-3.5" /></Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) closeDialog(); else setOpen(true); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>{editId ? "Edit Zone" : "Create Zone"}</DialogTitle></DialogHeader>
