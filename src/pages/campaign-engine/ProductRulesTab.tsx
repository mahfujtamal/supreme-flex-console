...
export default function ProductRulesTab({ campaignId }: { campaignId: string }) {
  ...
  // For PERCENT: which components to apply
  const [selectedComponents, setSelectedComponents] = useState<string[]>([]);
  // For FLAT: per-component breakdown
  ...
  // Fetch price components for selected product (latest active price version)
  const { data: productComponents } = useQuery({
    queryKey: ["product_price_components", productId],
    enabled: !!productId && ruleType === "DISCOUNT",
    queryFn: async () => {
      // Get the latest active price version for this product
  ...
  const { data: rules, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("*, products(product_name, product_category), campaign_discount_mappings(*)")
  ...
  // Compute resolved absolute discount for PERCENT type
  ...
  // Flat breakdown total
  const flatBreakdownTotal = flatBreakdown.reduce((s, r) => s + (parseFloat(r.amount) || 0), 0);

  // Validate flat breakdown: no component discount exceeds its price
  ...
  const addRule = useMutation({
    mutationFn: async () => {
      ...
      const { data: product, error: pErr } = await supabase
        .from("products").select("is_exclusive").eq("product_id", productId).single();
      if (pErr) throw pErr;
      if (ruleType === "EXCLUSIVE" && !product.is_exclusive) throw new Error("Product is not Exclusive.");
      if (ruleType !== "EXCLUSIVE" && product.is_exclusive) throw new Error("Exclusive products cannot be used with standard rules.");
          ...
          <div className="space-y-4 py-2">
            ...
            {ruleType === "DISCOUNT" && (
              <>
                ...
                {/* PERCENT: select which components */}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {flatBreakdown.map((fb, idx) => {
                        ...
                        return (
                          <div key={fb.component_name} className="flex items-center gap-3">
                            <span className="w-20 text-sm font-medium">{fb.component_name}</span>
                            <Input
                              type="number"
                              step="0.01"
                              min="0"
                              max={max}
                              placeholder={`Max ${formatBDT(max)}`}
                              value={fb.amount}
                              onChange={(e) => updateFlatAmount(idx, e.target.value)}
                              className="flex-1"
                            />
                            <span className="text-xs text-muted-foreground w-20 text-right">of {formatBDT(max)}</span>
                          </div>
                        );
                      })}
                    </div>
